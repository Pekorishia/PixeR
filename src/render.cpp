...
{  
    ...
    // Last, return the Z component if the ray hit the sphere 
    if (delta >= 0)
        return std::abs (c_.z());   
...
/*  
 * Returns de euclidian distance between two points
 *  
 */ 
float euclidian_distance (const point3 & u_, const point3 & v_)
{ 
    float distance = sqrt(pow (u_.x() - v_.x(), 2) + pow (u_.y() - v_.y(),2) + pow (u_.z() - v_.z(),2)); 
    return distance;
} 

/*  
 *  Find the resulting color of the interpolation within the circle
 *  
 * TODO: Doesn't work yet
 */ 
//rgb radial_interpolation (const Ray & r_, const point3 & c_, float radius_, const rgb & center, const rgb & border)
//{ 
    // 1) Get the point that the ray collides
    //point3 u_ ();

    // 2) Get the euclidian distance between the center and the ray's point 
    //float distance = euclidian_distance(u_, c_); 

    // 3) Verify if the distance isn't within radius
    //if (distance > radius_)
    //    return rgb (-1, -1, -1);

    // 3) normalize the distance based on the radius, to the range [0;1]     
    //auto t= 0.5+( ray_x*0.25 );     

    // 4) use linear interpolation (lerp) between the colors that compose the background     
    //rgb result = border*(1 - t) + center*(t);  
//}
